...
# modified 2017-08-14 by Pieter Robberechts in context of implementing QLAD system at DNS Belgium
# update staging stats is outcommented because of issue with old Impala version supported on AWS (1.2.4)

#Impala shell to stdout add a hidden shell escape character, this variable prevents this
export TERM=linux
...
for partition in $(impala-shell $IMPALA_OPTS -i $IMPALA_NODE -q "select year,month,day,server
                    ...
                    order by year,month,day,server desc
                    limit 10;" --output_delimiter=, --quiet --delimited)
do
    ...
    # outcommented just like the cron for run_05_update_stats_staging, reason :
