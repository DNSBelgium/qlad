...
enum { POINTS_COUNT = 50000, WINDOW_SIZE = 300 };
...
int main( int argc, char** argv )
{
	...
	if ( agg2plot.str() == a2plot.str() ) {
		::std::cout << "TEST PASSED! Aggregation: 1->2\n";
	} else {
		::std::cout << "TEST FAILED! Aggregation: 1->2\n";
		++ret;
	}

	if ( agg4plot.str() == a4plot.str() ) {
		::std::cout << "TEST PASSED! Aggregation: 1->4\n";
	} else {
		::std::cout << "TEST FAILED! Aggregation: 1->4\n";
		++ret;
	}

	if ( agg4bplot.str() == a4plot.str() ) {
		::std::cout << "TEST PASSED! Aggregation: 2->4\n";
	} else {
		::std::cout << "TEST FAILED! Aggregation: 2->4\n";
		++ret;
	}
