...
class DomainQuery extends Component {
  ...
  handleSubmit(event) {
    ...
    if (noErrors) {
      ...
      let sql = ` SELECT *
        FROM ${table}
        WHERE domainname='${this.state.domain}'
  ...
  render() {
    return (
        <Form plain={true} onSubmit={this.handleSubmit}>
          <FormField label="Domainname"
              style={{"width":"100%"}}
              error={this.state.errors.domain}>
            <TextInput placeHolder='example.be' onDOMChange={(e) => this.handleInputChange("domain", e.target.value)} />
          </FormField>
